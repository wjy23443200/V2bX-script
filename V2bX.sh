...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/wjy23443200/V2bX-script/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/wjy23443200/V2bX-script/master/install.sh) $version
    if [[ $? == 0 ]]; then
